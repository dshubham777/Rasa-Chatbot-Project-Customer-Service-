...
from rasax.community.constants import (
    CONFIG_FILE_TERMS_KEY,
    DEFAULT_RASA_ENVIRONMENT,
    RASA_X_DOCKERHUB_TAGS_URL,
)

if typing.TYPE_CHECKING:
    from rasa.utils.endpoints import EndpointConfig
    from multiprocessing import Process, BaseContext, Queue  # type: ignore
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(
                RASA_X_DOCKERHUB_TAGS_URL, timeout=timeout_sec
            ) as resp:
...
def mp_context() -> "BaseContext":
    ...
    return multiprocessing.get_context(config.MP_CONTEXT)  # type: ignore


def _run_in_process_target(
    fn: Callable[..., Any],
    telemetry_queue: Optional["Queue"],
    ...
    *args: List[Any],
    ...
    """
    from rasax.community import telemetry, config
    ...
    # Set the telemetry queue for this process
    telemetry.set_events_queue(telemetry_queue)

    # Set the telemetry write key for this process
    config.telemetry_write_key = telemetry_write_key

    # Set the LOCAL_MODE variable for this process
...
def run_in_process(
    ...
    p = mp_context().Process(
        ...
        args=(fn, telemetry_queue, config.telemetry_write_key, config.LOCAL_MODE)
        + args,
        daemon=daemon,
    )
    p.start()
